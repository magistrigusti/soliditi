...
contract Crowd {
  ...
  function claim(uint _id) external {
    Campaign storage campagn = campaigns[_id];
    require(msg.sender == campaign.owner, "not an owner");
  ...
  function refund(uint _id) external {
    ...
    require(block.timestamp > campaign.endAt, "not ended");
    require(campaign.pledged < campaign.goal, "reached gaol");

    uint pledgedAmmount = pledges[_id][msg.sender];
    pledges[_id][msg.sender] = 0;
    token.transfer(_id, msg.sender, pledgedAmount);
